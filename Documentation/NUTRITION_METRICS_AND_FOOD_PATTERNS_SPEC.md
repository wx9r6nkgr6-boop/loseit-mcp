...
Research/design pass, 2026-09-25; updated for the subsequent implementation instruction on the same date. The user authorized the implementation pass, including a weekly job only after safety and idempotence checks. All dates below use `America/New_York` calendar dates. This is a product specification, not individualized medical advice. See `NUTRITION_METRICS_IMPLEMENTATION.md` for implemented scope and measured results.

## 1. Executive summary and evidence labels

**Approved design.** The two views are **Nutrients** and **Food Patterns**, with a shared persistent period selector. Default prominent cards are **Protein, Fiber, Total Sugar**. Total Sugar is informational and is never substituted for Added Sugar. Added Sugar is a coverage-gated candidate and does not automatically replace a default card. Calories and Weight remain secondary context. Daily rendering is deterministic and independent of weekly research.
...
| Total sugar | **Trend/informational**; no universal limit | Explains sweet-food pattern but includes fruit/milk sugars; CW/LW/30 mean, no percent/status [E/I, R2] | Third prominent card; cannot support added-sugar inference. |
...
**Weekly research stage [P].** Use a separate local weekly LaunchAgent/entry point rather than extending the daily critical path. The existing local scheduled architecture and repository lock can support a distinct weekly job with its own idempotent audit run, no duplicate research of unchanged source identity+formulation fingerprint, and a retry checkpoint. Rank new/ambiguous foods by occurrences and calorie exposure, review mixed dishes and obvious false positives, consult exact manufacturer/USDA/restaurant/recipe evidence where useful, record source URL/date/basis/limitations, and commit only defensible versioned classifications. Expensive or uncertain cases remain unknown/directional or become user questions. Audit changes against prior classifications and regenerate snapshot after accepted updates. The implementation pass permits installation after manual/repeat runs, raw-data checks, and provider-failure checks. The installed job is local-only: approval review rejected sending private food names, brands, and occurrence counts to USDA. External exact-match research requires separate authorization. Harbor is not part of this local SQLite/iCloud execution path. [I/P]

## 14. Dashboard information architecture and customization readiness

Default Nutrients order: **Protein**, **Fiber**, **Total Sugar**. Total Sugar is informational, with no limit or graded status. Secondary list includes Added Sugar availability, Saturated Fat, Sodium, Calories, Total Fat, Carbohydrates, Cholesterol, and Weight Trend. [P] Calories remain easily accessible, but the user already sees them in Lose It. Food Patterns leads with Vegetables, Fruit, Seafood/Fatty fish, then Whole Grains, Legumes, Nuts/Seeds, and exploratory Plant Variety. Each card has a plain-language definition, period value, prior-period comparison, status or `Informational`, and quality/freshness affordance.
...
The implemented data contract is documented in `NUTRITION_METRICS_IMPLEMENTATION.md`. Schema version 9 adds append-only food-pattern evidence, added-sugar evidence and pilot history, daily allowance observations, and weekly audit runs. It preserves the Resolved Food Library and all raw Lose It rows. A reusable period/metric engine feeds both Streamlit views and the sanitized iCloud snapshot.

The 30-identity added-sugar pilot stored two historically applicable generic zeros and two current manufacturer-label zeros. The latter are date-scoped and do not backfill earlier logs. Rolling 30-day known coverage is 52/234 occurrences (22.2%) and 10.4% of source calories; Added Sugar remains unscored and Total Sugar remains the default third card. [I] [USDA plain-food guidance](https://aglab.ars.usda.gov/projects-nutrition-corner/take-healthy-eating-challenge), [Great Value broccoli listing](https://www.walmart.com/ip/735585383), and [Dymatize Cocoa Pebbles label](https://dymatize.com/products/iso100-cocoa-pebbles) support the stored zero assertions, subject to the identity and date limits recorded in the database.

The initial food-pattern pass created 13 assertions across 11 canonical foods. It intentionally leaves most mixed dishes and trace ingredients unknown. Vegetable subgroup evidence includes broccoli as dark-green, consistent with [USDA MyPlate](https://www.myplate.gov/web/web/eat-healthy/vegetables). Category classification coverage below 50% is labelled **Incomplete** even when some directional evidence is present. Verified quantity remains unavailable unless an exact portion equivalent is recorded.

The daily update now applies deterministic classification and computes all three periods before publication. The separate Sunday 11:30 AM LaunchAgent performs a local-only weekly audit under the existing update lock. A real manual run and immediate repeat left one audit row and no duplicate evidence. Exact-match USDA provider code exists for a future separately authorized manual research run; the scheduled job does not invoke it.

## 16. Decisions resolved and deferred

1. **Protein:** 150 g/day central target, 130 g/day On Track floor, no extra status above 150 g.
2. **Third default card:** Total Sugar, explicitly informational. Added Sugar never auto-promotes.
3. **Food Patterns:** Verified quantities and directional evidence remain separate; mixed-dish amounts are unknown without a defensible mapping.
4. **Status bands:** §5's five product labels are implemented for protein, fiber, sodium, saturated fat, and coverage-qualified added sugar; macro ranges, total sugar, cholesterol, and weight remain informational by design.
5. **Weekly cadence:** Sunday 11:30 AM local, after the 10:00 AM daily update. External research is disabled in the scheduled job following approval review. A future externally researching run requires approval of private food-name/brand egress.
6. **Home customization:** Stable metric keys, semantic types, and prominent order exist; pin/hide/reorder UI remains future work.
7. **Workout app reuse:** Consume the JSON-ready `read_metric_dashboard` result through a future explicit integration; no workout app or device code was modified.

## 17. Source/data safety and verification

A SQLite backup was created before migration at `/tmp/loseit-nutrition-pre-metrics.sqlite3` (SHA-256 `3be4dba9b876897ee33709c3d8db7f72dd3832be5922b17ed9abcf2625bd9118`). The schema-8-to-9 upgrade was tested on a copy, then applied to the live repository. Post-change comparison against the backup found all 280 raw diary snapshots, 16 raw weight snapshots, and 1,262 food occurrence rows exactly equal; database integrity is `ok`. `sources/`, raw Lose It files, the MCP read-only allowlist, and the workout app were not modified. Existing untracked `missing_coverage.json` was intentionally excluded. Device deployment: iPhone N/A, iPad N/A, Apple Watch N/A.

The static snapshot includes no credentials, cookies, authentication headers, raw diary payloads, or internal research attempts. The existing publication privacy scan was extended and the final iCloud JSON/HTML were scanned after publication.

## 18. Remaining research limits

The actual exercise-adjusted daily Lose It allowance remains unvalidated. Bundled GWT schemas contain `DailyBudget`, `DailyBudgetContext`, and `DailyLogGoalsState`; a fixture exposes a goal-like value and burn metrics, but their relationship to the displayed final allowance is not established. None of the 280 stored diary payloads contains an allowance/budget/adjustment field. No new Lose It protocol call was made. Calories therefore show intake only with **Informational** status. The allowance table accepts only `loseit_validated_final` provenance for scoring if a safe capture path is later proven.

Added-sugar label research for sweetened packaged foods remains limited by product/version and serving mismatches. In particular, a current manufacturer label is not proof of a past package. The 22.2% occurrence / 10.4% calorie coverage is materially below the 90%/90% gate. Food-pattern absence means unclassified, not zero intake; the exploratory plant count is not a clinical score. Weight unit remains null in live data, so a unit-based loss rate is withheld. External USDA research was blocked by automatic approval review because it would send private food names, brands, and occurrence counts; no indirect run was attempted after that rejection.
